...
export default function ContactModal({ isOpen }: {
    ...
    return (
        <dialog ref={dialogRef}>
            <div className={`grid place-items-center fixed w-full h-full left-0 top-0 ${isOpen ? 'animate-fadeIn' : 'animate-fadeOut'} bg-[#0006]`}>
                <div className={`${isOpen ? 'animate-slideIn' : 'animate-slideOut'} bg-white rounded-lg w-full max-w-xl m-4 overflow-hidden shadow-lg`}>
                    ...
                    <div className="relative">
                        <form onSubmit={handleSubmit} className={`${state.succeeded ? 'blur-sm' : ''} p-6 flex flex-col gap-2 relative`}>
                            ...
                            <input
                                ...
                                disabled={!state.succeeded && !state.submitting && email && message && /^[^@\s]+@[^@\s]+$/.test(email) ? undefined : true}
